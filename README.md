...
- les caches de navigation sont séparés : les requêtes `navigate` (pleine page) utilisent `pages-navigate` (NetworkFirst), les requêtes `RSC` utilisent le `defaultCache` de Serwist (`pages-rsc`). Un `setCatchHandler` sert la page d'accueil en cache quand une navigation échoue hors ligne, évitant `ERR_FAILED` ;
