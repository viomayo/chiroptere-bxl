...
- les caches de navigation sont séparés : les requêtes `navigate` (pleine page) sont servies depuis `pages-navigate` (NetworkFirst), les requêtes `RSC` depuis `pages-rsc`. Chaque navigation RSC en ligne déclenche aussi la mise en cache de la page HTML complète, garantissant que les deux types de navigation fonctionnent hors ligne ;
