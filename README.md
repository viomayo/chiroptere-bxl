...
- le cache de navigation unifie les requêtes `navigate` (pleine page) et `RSC` (navigation client-side) dans un seul cache `pages-navigation` avec `NetworkFirst`, évitant les erreurs `ERR_FAILED` quand le type de navigation diffère entre visite en ligne et hors ligne ;
